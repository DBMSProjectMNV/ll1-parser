...
function grammarChanged() {
    ...
    displayTables();
    $element('parseButton').removeAttribute("disabled");
    $element('resetTreeButton').removeAttribute("disabled");
}
...
function displayTables() {
    ...
    $element('llTableHead').innerHTML = "<th>NT</th>";
...
*/
    $element('parsingTableRows').innerHTML += '<tr><td nowrap=\"nowrap\">' + state.stack.join(' ') + "</td><td nowrap=\"nowrap\">" + state.input.slice(index).join(' ') + "</td nowrap=\"nowrap\"><td></td></tr>\n";
    // $element('input').style.color = ok ? 'green' : 'red';
    $element('simulation').style.setProperty('--fill-color', ok ? '#0a0' : '#f00');
    ...
    */
    $element('parseButton').setAttribute("disabled", '');
}
...
function parseInputStep() {
    ...
    if (state.position >= state.input.length) {
        ...
        $element('stepButton').setAttribute("disabled", '');
        return;
    }
...
*/
            ...
            console.log("Expected " + top + ", got " + next);
            // $element('input').style.color = ok ? 'green' : 'red';
            $element('simulation').style.setProperty('--fill-color', ok ? '#0a0' : '#f00');
            alert("Parsing error!! Given string is invalid");
            $element('stepButton').setAttribute("disabled", '');
            $element('parsingTableRows').innerHTML += '<tr><td nowrap=\"nowrap\">' + "" + "</td><td nowrap=\"nowrap\">" + "" + "</td nowrap=\"nowrap\"><td>Error</td></tr>\n";
            return;
        }
        ...
        if (rule == undefined) {
            ...
            console.log("Syntax error when trying to expand elements at the top of the stack:\nNo valid transition from a nonterminal \"" + top + '\" with look ahead symbol \"' + next + "\"");
            // $element('input').style.color = ok ? 'green' : 'red';
            $element('simulation').style.setProperty('--fill-color', ok ? '#0a0' : '#f00');
            alert("Parsing error!! Given string is invalid");
            $element('stepButton').setAttribute("disabled", '');
...
/*
    ...
    resize($element('remInput'), 10);
*/
    trace.stack = state.stack.join(' ');
    trace.input = state.input.slice(state.position).join(' ');
    $element('parsingTableRows').innerHTML += '<tr><td nowrap=\"nowrap\">' + trace.stack + "</td><td nowrap=\"nowrap\">" + trace.input + "</td nowrap=\"nowrap\"><td>"+ trace.rule + "</td></tr>\n";
    // $element('input').style.color = ok ? 'green' : 'red';
    $element('simulation').style.setProperty('--fill-color', ok ? '#0a0' : '#f00');
}
...
function update(parent, cnode) {
    ...
    nodeEnter.append('text')
        ...
        .style("fill", function (d) {
            return grammar.terminals.has(d.label) ? 'var(--fill-color, #0a0)' : null;
        })
        .text(function (d) {
            return d.label;
        })
...
function resetParse() {
    $element('parseButton').removeAttribute("disabled");
    $element('stepButton').setAttribute("disabled", '');
    $element('parsingTableRows').innerHTML = '';
    d3.selectAll("svg > *").remove();
}
